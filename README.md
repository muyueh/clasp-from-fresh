...
gitGraph
  ...
  commit id: "secret-gate-outputs" tag: "work@HEAD"
```
...
stateDiagram-v2
    ...
    state "Secret gate outputs" as SecretOutputs
    ...
    SecretGate --> SecretOutputs: write available flag
    SecretOutputs --> GHA: control remaining steps
...
sequenceDiagram
    ...
    CI->>CI: Publish secret-availability outputs
    alt Secrets missing
        CI-->>Dev: Skip deploy + warn (no downstream steps run)
...
flowchart LR
    ...
    SecretCheck -->|writes| GateOutputs
    GateOutputs -->|controls if-conditions| Workflow
...
flowchart LR
    ...
    SecretGate -->|寫 available flag| Outputs
    Outputs -->|決定後續步驟| CI
...
* `.github/workflows/deploy-gas.yml` 會在 `main` 推送或手動觸發後執行。
  * Workflow 步驟：
    ...
    2. 在執行部署前確認 `CLASPRC_JSON` 與 `TAIPEI_500_FORM_SCRIPT_ID` Secrets 是否同時可用，並輸出 `available` flag 供後續 steps 判斷；若缺少任何一個就發出警告並跳過其餘步驟。
