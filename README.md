...
stateDiagram-v2
    ...
    state "Workflow queue" as Queue
    state "Secret check" as Secret
    state "Deploy" as Deploy
    state "Google Apps Script" as GAS
    Local --> GitHub: git push (main/work)
    GitHub --> Queue: trigger deploy workflow
    Queue --> Secret: hydrate CLASPRC_JSON
    Secret --> Deploy: clasp login --status OK
    Secret --> Local: rotate secret if invalid
    Deploy --> GAS: clasp push -f
...
sequenceDiagram
    ...
    Dev->>Repo: Commit secure deploy fixes
    Repo-->>CI: Push event on main/work
    CI->>Secret: Write ~/.clasprc.json
    Secret-->>CI: Credentials ready
    CI->>CI: npm install + clasp login --status
    CI->>GAS: clasp push -f apps-script/taipei-500-form
    GAS-->>CI: Deployment result
    CI-->>Dev: Workflow summary / rotate secret if needed
```

```mermaid
flowchart LR
    Dev[Developer] -->|clasp / git| Repo[Monorepo]
    Repo -->|CI trigger| Runner[Deploy Workflow]
    Runner -->|reads| Secret[GitHub Secret\nCLASPRC_JSON]
    Runner -->|matrix clasp push| GAS[Apps Script Project]
    subgraph Shared Tooling
        Repo
    end
    GAS -->|Form URLs & logs| Dev
```
